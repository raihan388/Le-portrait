            ...
            <div class="hidden md:block bg-white rounded-xl shadow-md overflow-hidden">
                ...
                @foreach ($orders as $order)
                    <div class="grid grid-cols-9 gap-4 p-4 border-b border-gray-200 hover:bg-gray-50 transition-colors">
                        <div class="text-center text-blue-600 font-medium truncate">{{ $order->code_order ?? 'N/A' }}
                        </div>
                        ...
                        <div class="text-center text-xs text-gray-700">{{ ucfirst($order->payment_method ?? 'N/A') }}
                        </div>
                        <div class="text-center">
                            @php
                                $statusColors = [
                        ...
                        <div class="text-center space-y-1">
                            ...
                            {{-- Tombol Terima hanya untuk pembeli saat status 'processing' --}}
                            @if (strtolower($order->order_status) === 'processing' && Auth::check() && Auth::user()->email === $order->email)
                                <form action="{{ route('orders.complete', $order->id) }}" method="POST"
                                    onsubmit="return confirm('Yakin barang sudah diterima?')">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
                                        class="mt-1 inline-flex items-center px-2 py-1 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors text-sm">
                        ...
                        <div class="order-grid">
                            ...
                            <div>
                                ...
                                @php
                                    ...
                                    $color =
                                        $statusColors[strtolower($order->order_status)] ?? 'bg-gray-100 text-gray-800';
                                @endphp
